...
function installHooks() {
    ...
    // 移除旧的 wcc 条目
    const filteredPre = preToolUseHooks.filter((e) => !e.hooks?.some((h) => h.command?.includes('wcc') || h.command?.includes('wechat-claude-code')));
    ...
    const filteredStop = stopHooks.filter((e) => !e.hooks?.some((h) => h.command?.includes('wcc') || h.command?.includes('wechat-claude-code')));
...
function removeHooks() {
    ...
    for (const key of Object.keys(hooks)) {
        hooks[key] = hooks[key].filter((e) => !e.hooks?.some((h) => h.command?.includes('wcc') || h.command?.includes('wechat-claude-code')));
    }
    settings.hooks = hooks;
...
function registerMcp(serverPath) {
    ...
    let mcp = {};
    try {
        mcp = JSON.parse(fs.readFileSync(mcpFile, 'utf-8'));
    }
    catch { /* 文件不存在，用空对象 */ }
