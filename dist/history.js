...
export function loadHistory() {
    try {
        return JSON.parse(fs.readFileSync(getHistoryFile(), 'utf-8'));
    }
    catch {
        return [];
    }
...
export function addHistory(userId, message) {
...
export function updateSummary(id, summary) {
    const entries = loadHistory();
...
export function getEntry(n) {
    const entries = loadHistory();
    if (entries.length === 0 || n < 1 || n > entries.length)
        return null;
