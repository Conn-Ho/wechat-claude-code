...
function resolvePatchTarget(claudePath) {
    ...
    // 大文件（原生二进制）：尝试找 npm 全局安装的 cli.js
    if (stat.size > 1_000_000) {
        ...
        try {
            const prefix = execSync('npm config get prefix', { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
            npmCliCandidates.push(path.join(prefix, 'lib', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js'));
        }
        catch { /* ignore */ }
        for (const p of npmCliCandidates) {
            if (fs.existsSync(p))
                return p;
        }
        return claudePath;
...
function isBinaryFile(filePath) {
    ...
    if (header[0] === 0x7F && header[1] === 0x45)
        return true;
    if (header[0] === 0xFE && header[1] === 0xED)
        return true;
...
function patchBinary(exePath) {
    ...
    for (const { desc, variants } of BINARY_PATCH_GROUPS) {
        ...
        if (!groupPatched && groupSkipped) {
            console.log(`  ⏭️  ${desc} — 已修补`);
            skipped++;
        }
        else if (!groupPatched && !groupSkipped) {
...
async function patchAst(exePath) {
    ...
    if (!fs.existsSync(acornPath)) {
        ...
        if (!resp.ok)
            throw new Error(`下载 acorn 失败: HTTP ${resp.status}`);
        ...
        for (const key in n) {
            ...
            if (val && typeof val === 'object') {
                if (Array.isArray(val))
                    val.forEach(child => findNodes(child, predicate, results));
                else
                    findNodes(val, predicate, results);
            }
        }
        return results;
    }
    ...
    const harborCalls = findNodes(ast, n => n.type === 'CallExpression' && n.arguments?.length === 2 &&
    ...
    if (markerLiterals.length > 0) {
        ...
        const enclosing = findNodes(ast, n => (n.type === 'FunctionDeclaration' || n.type === 'FunctionExpression') && n.start < markerPos && n.end > markerPos).sort((a, b) => (a.end - a.start) - (b.end - b.start));
    ...
    // Patch 3: UI notice
    const policyProps = findNodes(ast, n => n.type === 'Property' && n.key?.type === 'Identifier' && n.key?.name === 'policyBlocked' &&
        n.value != null && src(n.value).includes('channelsEnabled'));
    let noticePatched = false;
    if (policyProps.length > 0) {
        const propPos = policyProps[0].start;
        const noticeFuncs = findNodes(ast, n => (n.type === 'FunctionDeclaration' || n.type === 'FunctionExpression') && n.start < propPos && n.end > propPos).sort((a, b) => (a.end - a.start) - (b.end - b.start));
        if (noticeFuncs.length > 0) {
            const nf = noticeFuncs[0];
            // 直接返回固定对象，不调用原始函数，避免运行时类型错误
            const newBody = '{return{channels:[],disabled:!1,noAuth:!1,policyBlocked:!1,list:""}}';
...
function unpatch() {
    ...
    const backupPath = exePath + '.bak';
    if (!fs.existsSync(backupPath)) {
        console.error(`  未找到备份 ${backupPath}\n`);
        process.exit(1);
    }
    try {
