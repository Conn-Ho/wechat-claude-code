...
export function loadTrustedUsers() {
    try {
...
export function addTrustedUser(userId, nickname) {
...
/** 未设置白名单时返回 true（向后兼容） */
export function isUserTrusted(userId) {
    const users = loadTrustedUsers();
    if (users.length === 0)
        return true;
...
export function formatUsers() {
    const users = loadTrustedUsers();
    if (users.length === 0)
        return '未设置白名单（所有用户均可访问）';
    return users.map((u, i) => `${i + 1}. ${u.nickname ? `${u.nickname} ` : ''}(${u.userId.slice(0, 12)}...)`).join('\n');
}
