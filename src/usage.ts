...
export function loadUsage(): Record<string, DayUsage> {
  try {
    return JSON.parse(fs.readFileSync(getUsageFile(), 'utf-8')) as Record<string, DayUsage>;
  } catch {
    return {};
  }
}

function saveUsage(data: Record<string, DayUsage>): void {
  fs.writeFileSync(getUsageFile(), JSON.stringify(data, null, 2), 'utf-8');
}

export function trackMessage(type: 'sent' | 'received', text: string): void {
  const data = loadUsage();
  const d = today();
  if (!data[d]) data[d] = { date: d, messagesSent: 0, messagesReceived: 0, estimatedTokens: 0 };
  if (type === 'sent') data[d].messagesSent++;
  else data[d].messagesReceived++;
