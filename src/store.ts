 ...
 */
export function saveLastUser(data: LastUserContext): void {
 ...
 * 读取最近一次用户上下文
 */
export function loadLastUser(): LastUserContext | null {
  try {
    return JSON.parse(readFileSync(join(getStateDir(), LAST_USER_FILE), 'utf-8')) as LastUserContext;
  } catch {
    return null;
  }
}
