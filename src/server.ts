...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === 'login') {
    try {
      ...
      startPolling();
      return { content: [{ type: 'text' as const, text: `登录成功！账号 ID: ${result.accountId}` }] };
    } catch (err) {
  ...
  if (name === 'reply') {
    ...
    if (!userId || !contextToken || !content) {
      return { content: [{ type: 'text' as const, text: '缺少必填参数: user_id, context_token, content' }], isError: true };
    }

    const account = getActiveAccount();
    if (!account) {
      return { content: [{ type: 'text' as const, text: '未登录，请先使用 login 工具扫码登录' }], isError: true };
    }

    if (media && !fs.existsSync(media)) {
      return { content: [{ type: 'text' as const, text: `媒体文件不存在: ${media}` }], isError: true };
    }
    ...
    try {
      ...
      return { content: [{ type: 'text' as const, text: `已发送 ${chunks.length} 段文本${media ? ' + 1 个媒体文件' : ''}${mediaError}` }] };
    } catch (err) {
  ...
  if (name === 'notify') {
    const content = args?.content as string | undefined;
    if (!content) {
      return { content: [{ type: 'text' as const, text: '缺少 content 参数' }], isError: true };
    }
    try {
      await pushToUser(content);
      return { content: [{ type: 'text' as const, text: '通知已发送' }] };
    } catch (err) {
      return { content: [{ type: 'text' as const, text: `通知发送失败: ${String(err)}` }], isError: true };
    }
  }
