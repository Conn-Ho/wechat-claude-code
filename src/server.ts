...
import {
  watchApprovals, respondToLatest, respondToApproval,
  pendingApprovals, formatApprovalRequest,
} from './approval.js';
...
import {
  addJob, removeJob, getDueJobs, markJobRun,
  formatJobs, parseScheduleCommand,
} from './scheduler.js';
...
import {
  addProject, removeProject, findProject, getActiveProject,
  setActiveProject, formatProjects,
} from './projects.js';
...
const activeHistoryId = new Map<string, number>();
...
async function pushToUser(text: string, userId?: string, contextToken?: string): Promise<void> {
...
function startProgressTimer(userId: string, contextToken: string): void {
...
function stopProgressTimer(userId: string): void {
  const t = progressTimers.get(userId);
  if (t) { clearInterval(t); progressTimers.delete(userId); }
}
    ...
    if (t === MessageItemType.TEXT) {
      if (item.text_item?.text) {
        if (item.ref_msg) {
          ...
          if (refContent) parts.push(`[引用: ${refContent}]`);
        }
...
async function handleCommand(text: string, userId: string, contextToken: string): Promise<boolean> {
  const t = text.trim();
  ...
  if (approveM) {
    ...
    const decision = isAllow ? 'allow' : 'deny' as const;
    ...
    if (handled) await pushToUser(isAllow ? '✅ 已允许' : '❌ 已拒绝', userId, contextToken);
    else await pushToUser('暂无待审批操作', userId, contextToken);
    return true;
  }
  ...
  if (t === '/retry') {
    ...
    if (!entry) { await pushToUser('暂无历史', userId, contextToken); return true; }
  ...
  if (redoM) {
    ...
    if (!entry) { await pushToUser('历史记录不存在', userId, contextToken); return true; }
  ...
  if (getM) {
    ...
    const account = getActiveAccount();
    if (!account) { await pushToUser('未登录', userId, contextToken); return true; }
    try {
      if (!fs.existsSync(filePath)) { await pushToUser(`文件不存在: ${filePath}`, userId, contextToken); return true; }
      const stat = fs.statSync(filePath);
      if (stat.size > 10 * 1024 * 1024) { await pushToUser('文件超过 10MB，无法发送', userId, contextToken); return true; }
      // 文本文件直接发内容
  ...
  if (t === '/status' || t === '/状态') {
    ...
    if (pending > 0) {
      for (const [id, req] of pendingApprovals) lines.push(`  · ${req.tool_name} (${id.slice(0, 8)})`);
    }
...
server.setRequestHandler(CallToolRequestSchema, async (request: { params: { name: string; arguments?: Record<string, unknown> } }) => {
  ...
  if (name === 'login') {
    try {
      ...
      startPolling();
      return { content: [{ type: 'text' as const, text: `登录成功！账号: ${result.accountId}` }] };
    } catch (err) {
      return { content: [{ type: 'text' as const, text: `登录失败: ${String(err)}` }], isError: true };
    }
  }
  ...
  if (name === 'reply') {
    ...
    if (!userId || !contextToken || !content) {
      return { content: [{ type: 'text' as const, text: '缺少必填参数' }], isError: true };
    }
    const account = getActiveAccount();
    if (!account) return { content: [{ type: 'text' as const, text: '未登录' }], isError: true };
    if (media && !fs.existsSync(media)) return { content: [{ type: 'text' as const, text: `文件不存在: ${media}` }], isError: true };
    ...
    try {
      ...
      if (histId) { updateSummary(histId, content); activeHistoryId.delete(userId); }
      ...
      return { content: [{ type: 'text' as const, text: `已发送 ${chunks.length} 段${media ? ' + 媒体' : ''}` }] };
    } catch (err) {
      return { content: [{ type: 'text' as const, text: `发送失败: ${String(err)}` }], isError: true };
    }
  }

  if (name === 'notify') {
    const content = args?.content as string | undefined;
    if (!content) return { content: [{ type: 'text' as const, text: '缺少 content' }], isError: true };
    try {
      await pushToUser(content);
      return { content: [{ type: 'text' as const, text: '通知已发送' }] };
    } catch (err) {
      return { content: [{ type: 'text' as const, text: `发送失败: ${String(err)}` }], isError: true };
    }
  }
...
async function sendTypingIfPossible(account: AccountData, userId: string, contextToken: string, status: number): Promise<void> {
  try {
    ...
    if (ticket) await sendTyping(account.token, userId, ticket, status, account.baseUrl);
  } catch { /* ignore */ }
}
...
async function pollLoop(account: AccountData): Promise<void> {
  ...
  while (pollingActive && !pollingAbort?.signal.aborted) {
    try {
      ...
      if (resp.longpolling_timeout_ms) nextTimeoutMs = resp.longpolling_timeout_ms;
      ...
      for (const msg of resp.msgs ?? []) {
        ...
        try {
          ...
          if (config.typing_ticket) typingTicketCache.set(fromUser, config.typing_ticket);
        } catch { /* ignore */ }
        ...
        try {
          ...
          if (ticket) await sendTyping(account.token, fromUser, ticket, 1, account.baseUrl);
        } catch { /* ignore */ }
...
function startPolling(): void {
  ...
  pollLoop(account).catch(err => {
    if (!pollingAbort?.signal.aborted) process.stderr.write(`[wcc] Poll crashed: ${String(err)}\n`);
    pollingActive = false;
  });
}
